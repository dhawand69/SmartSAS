...
document.getElementById("adminAttendanceBody").innerHTML = <tr> <td colspan="10" style="text-align:center; padding: 40px; color: gray;"> ⚙️ Set your filters and click "Load Attendance" to view data </td> </tr> ;
...
const tr = document.createElement("tr");
tr.innerHTML = <td><input type="checkbox" class="student-checkbox" value="${ student.id }" onchange="handleCheckboxChange(this)" ${ isSelected ? "checked" : "" }></td><td style="cursor: pointer; color: var(--color-primary);" onclick="viewStudentAttendance(${ student.id }, '${student.roll_no}', '${student.first_name} ${student.last_name}')">${ student.roll_no || "" }</td><td>${student.first_name || ""} ${student.last_name || ""}</td><td>${ student.department || "" }</td><td>${ student.year || "" }</td><td><span class="status-badge" style="background:#eaf6fd; color:#2c5282;">Sem ${ student.semester || "" }</span></td><td><button class="btn btn-small btn-danger" onclick="deleteStudent(${ student.id })">Delete</button></td>;
tbody.appendChild(tr);
});
...
div.innerHTML = <div style="text-align:left;"><div style="font-weight:bold; color:var(--color-dark);">${ student.first_name } ${ student.last_name }</div><div style="font-size:12px; color:var(--color-gray);">${ student.roll_no }</div></div><label class="attendance-toggle"><input type="checkbox" class="attendance-checkbox" value="${ student.id }" ${ isChecked ? "checked" : "" }><span class="toggle-label">Present</span></label>;
